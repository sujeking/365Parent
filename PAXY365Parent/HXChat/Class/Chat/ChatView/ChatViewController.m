...
#warning 以下三行代码必须写，注册为SDK的ChatManager的delegate
    ...
    if (_chatter.length==11) {
        
        //设置头像和昵称
        NSString *getUserName = _chatter;
    ...
    if ([messagesArray count] > 0) {
        for (EMMessage *message in messagesArray) {
            ...
            if ([_delelgate respondsToSelector:@selector(nickNameWithChatter:)]) {
                ...
                //设置头像和昵称
                NSString *getUserName = model.username;
                NSString *saveUserName = [NSString stringWithFormat:@"User%@",getUserName];
                NSString *getNickHead = nil;
                NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
                if (defaults)
                ...
                model.nickName = getNickName;//showName?showName:model.username;
            }else {
...
-(void)sendAudioMessage:(EMChatVoice *)voice
{
    ...
    //设置头像和昵称
    NSString *getUserName = _conversation.chatter;
    NSString *saveUserName = [NSString stringWithFormat:@"User%@",getUserName];
    NSString *getNickHead = nil;
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    if (defaults)
