...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //设置头像和昵称
    NSString *getUserName = [self.dataSource objectAtIndex:indexPath.row];
...
#warning 以下代码为测试代码，默认用户体系中有一个符合要求的同名用户
        ...
        [self checkUsers:_textField.text];
        
        
    }
}

- (void)checkUsers:(NSString *)username
{
    //判断服务器是否存在
    @try {
        ...
        NSString *urlString = [NSString stringWithFormat:@"%@?platform=iOS&uid=%@&name=%@",CheckUserUrl,getUserID,username];
        ...
        NSLog(@"URL=%@",urlString);
        NSDictionary *dict = [DataSource fetchJSON:urlString];
        ...
        for (int i = 0; i < [array count]; i ++) {
            ...
            if ([getStatus isEqualToString:@"0"]) {
                ...
                [self.dataSource removeAllObjects];
                [self.dataSource addObject:getUserPhone];
