...
-(NSString *) sendFormAsyRequst:(NSString *) urlString
{
    ...
    request = [[ASIFormDataRequest requestWithURL:[NSURL URLWithString:self.requestURLString]] retain];
	  
    [request setUseCookiePersistence:YES];
	[request setTimeOutSeconds:timout];
...
- (void) requestFinished:(ASIHTTPRequest *) asiRequest
{
    ...
    //NSLog(@"异步返回数据%@",retString);
    if ([retString rangeOfString:@"[]"].location != NSNotFound) {
        
        //判断数据返回是否为空，为空清空如下：
        //异步返回数据{"result":"0","List":"[]"}
        NSLog(@"异步获取数据为空");
        NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
        [defaults setObject:@"0" forKey:@"cloudin_365paxy_request_data"];
    }
    else{
        NSLog(@"异步获取数据为真");
        NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
        [defaults setObject:@"1" forKey:@"cloudin_365paxy_request_data"];
    }
