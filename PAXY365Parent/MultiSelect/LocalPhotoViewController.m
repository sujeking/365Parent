...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    else if (self.selectPhotos.count <= 5){
        self.lbAlert.text=[NSString stringWithFormat:@"已经选择%ld张照片",(unsigned long)self.selectPhotos.count];
    } else {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"最多只能选择5张图片" message:nil delegate:nil cancelButtonTitle:@"我知道了" otherButtonTitles: nil];
        [alert show];
        [cell.btnSelect setHidden:YES];
