...
@interface WarningListViewController ()
...
-(void) viewDidAppear:(BOOL)animated
{
    ...
    if (defaults){
        ...
        if ([getMessageFlag isEqualToString:@"1"]) {
            ...
            segmentMenus.selectedSegmentIndex = 1;
...
- (void)viewDidLoad
{
    ...
    [segmentMenus insertSegmentWithTitle:@"周边" atIndex:0 animated:YES];
    [segmentMenus insertSegmentWithTitle:@"我的" atIndex:1 animated:YES];
...
-(void) sendGetDatas
{
    ...
    NSString *getCurrentUrl=nil;
    if ([getUserID isEqualToString:@"-1"]) {
        getCurrentUrl=[NSString stringWithFormat:@"%@?uid=%@&lat=%@&lng=%@",WarningByMeListUrl,getUserID,getLat,getLng];
    }
    else{
        getCurrentUrl=[NSString stringWithFormat:@"%@?uid=%@&lat=%@&lng=%@",WarningByMeListUrl,getUserID,getLat,getLng];
    }
...
-(void) loadmore:(UIButton *) sender
{
    ...
    if ([getUserID isEqualToString:@"-1"]) {
        pageUrl=[NSString stringWithFormat:@"%@?PageSize=20&Page=%d&uid=%@&lat=%@&lng=%@",WarningByMeListUrl,currentPage,getUserID,getLat,getLng];
    }
    else{
        pageUrl=[NSString stringWithFormat:@"%@?PageSize=20&Page=%d&uid=%@&lat=%@&lng=%@",WarningByMeListUrl,currentPage,getUserID,getLat,getLng];
    }
