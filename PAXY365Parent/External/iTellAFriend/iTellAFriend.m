...
{
    if (alertView==YES) {
        UIAlertView* alertView = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"评价应用", @"") message:[NSString stringWithFormat:NSLocalizedString(@"如果你喜欢我们的应用 %@, 就给我们评价一下，感谢你对我们的支持!", @""), self.applicationName] delegate:self cancelButtonTitle:NSLocalizedString(@"不, 谢谢", @"") otherButtonTitles:NSLocalizedString(@"评价", @""), nil];
        alertView.tag = 2;
        [alertView show];
...
- (NSString *)message
{
    ...
    return @"请检查此应用是否已发布到苹果App Store:";
}
