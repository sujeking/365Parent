...
- (void)iniView:(NSString *)image
{
    ...
    NSLog(@"SetImage：%@",setImage);
    
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    [defaults setObject:setImage forKey:@"cloudin_365paxy_show_image_homework"];
    ...
    lblTitle.text = [NSString stringWithFormat:@"教师：%@",setName];
    ...
    if([image isEqualToString:@"icon_sns_love_on"]){
     
        UIButton *btnLove = [[UIButton alloc] initWithFrame: CGRectMake(250,205,60,30)];
...
- (void)tapImage:(UITapGestureRecognizer *)tap
{
    ...
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    if (defaults){
        showImageURL = [defaults objectForKey:@"cloudin_365paxy_show_image_homework"];
    }
