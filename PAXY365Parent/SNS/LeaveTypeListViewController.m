...
@interface LeaveTypeListViewController ()

@end

@implementation LeaveTypeListViewController
...
-(void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([object isKindOfClass:[ClasssEntity class]])
    {
        ...
        NSString *getID = data.ClassID;
        NSString *getName = data.ClassName;
        getID = [NSString stringWithFormat:@"%@",getID];
        getName = [NSString stringWithFormat:@"%@",getName];
        ...
        [defaults setObject:getID forKey:@"cloudin_365paxy_selected_leavetypeid"];
        [defaults setObject:getName forKey:@"cloudin_365paxy_selected_leavetype_name"];
        [defaults setObject:getName forKey:@"cloudin_365paxy_selectedtemp_leavetype_name"];
...
-(UITableViewCell *) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([object isKindOfClass:[ClasssEntity class]])
    {
        ...
        ClassCell *cell  = (ClassCell *)[self.tableView dequeueReusableCellWithIdentifier:identifier];
        
        if (cell == nil)
        {
            cell = [[[NSBundle mainBundle] loadNibNamed:@"ClassCell" owner:self options:nil] objectAtIndex:0];
            cell.selectionStyle = UITableViewCellSelectionStyleBlue;
        }
        
        ClasssEntity *news = (ClasssEntity *)object;
        cell.object = news;
...
-(void) sendGetDatas
{
    ...
    if (defaults)
    {
        getKeywords = [defaults objectForKey:@"cloudin_365paxy_keywords_class"];
    }
    ...
    NSString *getCurrentUrl=[NSString stringWithFormat:@"%@?flag=%@&keywords=%@",LeaveTypeListUrl,setFlag,getKeywords];
...
-(void) loadmore:(UIButton *) sender
{
    ...
    NSString *pageUrl = [NSString stringWithFormat:@"%@?PageSize=20&Page=%d&flag=%@&keywords=%@",LeaveTypeListUrl,currentPage,setFlag,getKeywords];
...
-(void) netFinish:(NSDictionary *)jsonString withUserInfo:(NSDictionary *)userInfo
{
    ...
    if (jsonString)
    {
        temparray = [ParseJson getClassList:jsonString];
    }
...
- (void)updateSearchString:(NSString*)searchString
{
    ...
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    [defaults setObject:searchString forKey:@"cloudin_365paxy_keywords_class"];
