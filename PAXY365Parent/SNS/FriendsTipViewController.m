...
@interface FriendsTipViewController ()

@end

@implementation FriendsTipViewController
...
-(void) viewDidAppear:(BOOL)animated
{
    ...
    if (defaults){
        getMessageFlag = [defaults objectForKey:@"cloudin_365paxy_message_reviewfriends"];
        
        if ([getMessageFlag isEqualToString:@"1"]) {
            
            [self showMessage:@"操作成功，又新添了一名好友！"];
...
-(void)showMessage:(NSString *)message
{
    ...
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    [defaults setObject:nil forKey:@"cloudin_365paxy_message_reviewfriends"];
}
...
-(void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([object isKindOfClass:[ContactsEntity class]])
    {
        ...
        FriendsReviewViewController *nextController = [FriendsReviewViewController alloc];
...
-(UITableViewCell *) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([object isKindOfClass:[ContactsEntity class]])
    {
        ...
        FriendsTipCell *cell  = (FriendsTipCell *)[self.tableView dequeueReusableCellWithIdentifier:identifier];
        
        if (cell == nil)
        {
            cell = [[[NSBundle mainBundle] loadNibNamed:@"FriendsTipCell" owner:self options:nil] objectAtIndex:0];
...
-(void) sendGetDatas
{
    ...
    NSString *getCurrentUrl=[NSString stringWithFormat:@"%@?uid=%@&flag=%@",FriendsListUrl,getUserID,setFlag];
...
-(void) loadmore:(UIButton *) sender
{
    ...
    NSString *pageUrl = [NSString stringWithFormat:@"%@?PageSize=20&Page=%d&uid=%@&flag=%@",FriendsListUrl,currentPage,getUserID,setFlag];
