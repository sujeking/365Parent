...
@interface BehaveListViewController ()

@end

@implementation BehaveListViewController
...
-(void) viewDidAppear:(BOOL)animated
{
    ...
    if (defaults){
        getFlagMessage = [defaults objectForKey:@"cloudin_365paxy_message_behave"];
        if ([getFlagMessage isEqualToString:@"1"]) {
            [self showMessage:@"恭喜你，在校表现发布成功！"];
        }
...
-(void)showMessage:(NSString *)message
{
    ...
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    [defaults setObject:nil forKey:@"cloudin_365paxy_message_behave"];
}
...
-(UITableViewCell *) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([object isKindOfClass:[MessageEntity class]])
    {
        ...
        static NSString *identifier1 = @"BehaveCell2";
        ...
        if ([getDate isEqualToString:@"1"]) {
            //隐藏日期
            cell  = (BehaveCell *)[self.tableView dequeueReusableCellWithIdentifier:identifier0];
        }
        else{
            cell  = (BehaveCell *)[self.tableView dequeueReusableCellWithIdentifier:identifier1];
        }
        ...
        if (cell == nil)
        {
            ...
            if ([getDate isEqualToString:@"1"]) {
                //隐藏日期
                cell = [[[NSBundle mainBundle] loadNibNamed:@"BehaveCell" owner:self options:nil] objectAtIndex:0];
            }
            else{
                cell = [[[NSBundle mainBundle] loadNibNamed:@"BehaveCellTwo" owner:self options:nil] objectAtIndex:0];
...
- (void)gotoAdd{
    ...
    BehaveAddViewController *vc = [[BehaveAddViewController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}
...
-(void) sendGetDatas
{
    ...
    if (defaults)
    {
        ...
        getKeyword = [defaults objectForKey:@"cloudin_365paxy_keywords_behave"];
    }
    ...
    NSString *getCurrentUrl=[NSString stringWithFormat:@"%@?uid=%@&sid=%@&cid=%@&gid=%@&channelid=%@&flag=%@&stime=%@&etime=%@&keyword=%@",MessageListUrl,getUserID,getSchoolID,getClassID,getGradeID,setChannelID,setFlag,getSTime,getETime,getKeyword];
...
-(void) loadmore:(UIButton *) sender
{
    ...
    NSString *pageUrl = [NSString stringWithFormat:@"%@?PageSize=20&Page=%d&uid=%@&sid=%@&cid=%@&gid=%@&channelid=%@&flag=%@&stime=%@&etime=%@&keyword=%@",MessageListUrl,currentPage,getUserID,getSchoolID,getClassID,getGradeID,setChannelID,setFlag,getSTime,getETime,getKeyword];
