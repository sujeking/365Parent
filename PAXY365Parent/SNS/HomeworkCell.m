...
-(void) layoutSubviews
{
    ...
    if ([object isKindOfClass:[MessageEntity class]])
    {
        ...
        lblName.text = data.SendUserName;
        lblName.textColor = TxtBlack;

        lblCourse.text = [NSString stringWithFormat:@"%@",data.CourseName];
        lblCourse.textColor = TxtWhite;
        if ([data.CourseName isEqualToString:@"语文"]) {
             lblCourse.backgroundColor = SNSItem1;
        }
        else if([data.CourseName isEqualToString:@"数学"]){
            lblCourse.backgroundColor = SNSItem2;
        }
        else if([data.CourseName isEqualToString:@"英语"]){
            lblCourse.backgroundColor = SNSItem3;
        }
        else{
            lblCourse.backgroundColor = TxtBlue;
        }
