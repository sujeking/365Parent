...
@interface InfoListNewViewController () <SDCycleScrollViewDelegate>


@end

@implementation InfoListNewViewController
...
- (void)handleSwipes:(UISwipeGestureRecognizer *)sender
{
    ...
    if (sender.direction == UISwipeGestureRecognizerDirectionLeft) {
        //left
        NSLog(@"left:%ld",(long)segmentMenus.selectedSegmentIndex);
        
        if (segmentMenus.selectedSegmentIndex==0) {
            
            //[Animations fadeIn:self.tableView andAnimationDuration:1.0 andWait:YES];
            [segmentMenus setSelectedSegmentIndex:1];
        ...
        else if (segmentMenus.selectedSegmentIndex==1) {
            
            //[Animations fadeIn:self.tableView andAnimationDuration:1.0 andWait:YES];
            [segmentMenus setSelectedSegmentIndex:2];
        ...
        else if (segmentMenus.selectedSegmentIndex==2) {
            
            //[Animations fadeIn:self.tableView andAnimationDuration:1.0 andWait:YES];
            [segmentMenus setSelectedSegmentIndex:0];
    ...
    if (sender.direction == UISwipeGestureRecognizerDirectionRight) {
        //right
        NSLog(@"right:%ld",(long)segmentMenus.selectedSegmentIndex);
        if (segmentMenus.selectedSegmentIndex==0) {
...
- (void)iniView
{
    ...
    segmentMenus = [[UISegmentedControl alloc]
                    initWithFrame:CGRectMake(5, 5, 310, 29)];
    [segmentMenus insertSegmentWithTitle:@"热点聚焦" atIndex:0 animated:YES];
    [segmentMenus insertSegmentWithTitle:@"校园新闻" atIndex:1 animated:YES];
    [segmentMenus insertSegmentWithTitle:@"通知公告" atIndex:2 animated:YES];
    ...
    imagePlayerView.frame = CGRectMake(0, 40, 320, 150);
    ...
    // 广告标题
    lblAdsTitle = [[UILabel alloc] init];
    lblAdsTitle.frame = CGRectMake(0, 160, 320, 30);
    lblAdsTitle.backgroundColor = [UIColor colorWithRed:120/255.0 green:120/255.0 blue:120/255.0 alpha:0.4];
    lblAdsTitle.textAlignment = NSTextAlignmentCenter;
    lblAdsTitle.font = [UIFont systemFontOfSize:14.0f];
    lblAdsTitle.textColor = TxtWhite;
    //[headerView addSubview:lblAdsTitle];
    ...
    footView.backgroundColor = WhiteBgColor;
...
- (void)loadAdsData
{
    @try {
        ...
        //从服务端获取广告
        NSString *urlString = [NSString stringWithFormat:@"%@?flag=4",AdsListUrl];
        NSLog(@"URL=%@",urlString);
        ...
        for (int i = 0; i < [loginStatus count]; i ++) {
            ...
            NSLog(@"LinkType: %@,title: %@",getLinkType,getNewsTitle);
        ...
        //网络加载 --- 创建带标题的图片轮播器
        SDCycleScrollView *cycleScrollView = [SDCycleScrollView cycleScrollViewWithFrame:CGRectMake(0, 40, w, 150) imageURLStringsGroup:nil]; // 模拟网络延时情景
...
{
    NSString *getTitle = [adsTitles objectAtIndex:index];
    NSLog(@"Title:%@",getTitle);
    lblAdsTitle.text = getTitle;
